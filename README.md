...
`variables:` parameter, and that pipe's own container sees literally `AWS_ACCESS_KEY_ID`. Array
variables use Bitbucket's own flat convention directly -- `SOME_VAR_COUNT=2`, `SOME_VAR_0=...`,
`SOME_VAR_1=...` -- there is no orb-specific array syntax to learn.
...
`map-env` sets these before the pipe runs. "Confirmed" means the mapping (or the variable's real
usage) was verified against Atlassian's docs and/or real pipe source; "synthesized" means there
is no CircleCI equivalent and a placeholder is generated purely so the pipe does not crash on a
missing variable.
...
- **Bitbucket's own output-variables mechanism catches less than you'd expect**: real pipes
  ...
  zero extra plumbing -- you don't need this orb's output-variable mechanism for that at all.
