...
# Mandatory eligibility gate for the "pipe image as the job container" (native primary container)
# path. Runs FIRST, before checkout/attach_workspace, so an ineligible image fails fast with a
# specific, actionable reason instead of a confusing mid-job break (a checkout that dies partway
# through because git is missing, or a pipe binary that dies on its first HTTPS call because the
# CA bundle is empty).
#
...
# --- 1. Docker-daemon requirement (the plugins/docker-style case) -----------------------------
#
# An image like plugins/docker is built FROM the official `docker` image and its own ENTRYPOINT is
...
# tool: the fix is "use the machine-executor `pipe` job instead", not "add a missing binary to
# your image".
#
...
# own dockerd. A pipe that merely shells out to a bare `docker` CLI and assumes some EXTERNAL
# daemon is reachable via a pre-set $DOCKER_HOST leaves no such static signature and is NOT
# reliably detectable from inside the container filesystem alone -- needing a live daemon at
# runtime is a behavior, not a file. Documented here and in the README rather than silently
# claimed as caught.
...
# receives, and both attach_workspace and the pipe's own network calls need real CA certs for
...
for candidate_ca in /etc/ssl/certs/ca-certificates.crt /etc/ssl/certs/ca-bundle.crt \
    ...
    if [ -f "${candidate_ca}" ]; then
        # A CA bundle "present but a stub" is real, verified reality
        # (bitbucketpipelines/demo-pipe-bash) -- so existence alone is not enough; require a size
        # a real bundle would actually have. Debian bundles run several hundred KB; even a
        # minimal set is tens of KB. 1024 bytes is a generous floor that a genuine (if pruned)
        # bundle clears easily and an empty/stub placeholder file does not.
...
if [ -z "${ca_bundle_found}" ]; then
    fail "missing-ca-certificates" "no usable CA certificate bundle was found in this image (checked /etc/ssl/certs/ca-certificates.crt, /etc/ssl/certs/ca-bundle.crt, /etc/pki/tls/certs/ca-bundle.crt, /etc/ssl/cert.pem -- each must exist AND be larger than 1024 bytes, since a present-but-empty/stub bundle file is a real case this orb has seen). Without real CA certificates, HTTPS calls this job needs (CircleCI's own API for attach_workspace/persist_to_workspace, and most pipe backends) will fail with certificate-verification errors."
fi
